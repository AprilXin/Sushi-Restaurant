...
public class ChefImpl implements Chef, BeltObserver {
	...
	private double consumed_amount;
	private double spoiled_amount;
		...
		if (e.getType() == BeltEvent.EventType.PLATE_CONSUMED) {
			...
			if (plate.getChef() == this) {
				...
				for (int i = 0; i < plate.getContents().getIngredients().length; i++) {
					consumed_amount += plate.getContents().getIngredients()[i].getAmount();
				}
			}
			...
			if (plate.getChef() == this) {
				...
				for (int i = 0; i < plate.getContents().getIngredients().length; i++) {
					spoiled_amount += plate.getContents().getIngredients()[i].getAmount();
				}
			}
